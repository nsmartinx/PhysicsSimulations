...
class Collision(ShowBase):
    def __init__(self):
        ...
        global t, cube0, plane0, fps, timeSinCol, gravityHold
    ...
    def physicsUpdate(self, task):
        ...
        global timeSinCol, projNor, gravityHold
        projNor = (0,0,0)
        ...
        #find points touching plane and storing in array
        for x in cube0.points:
            disToPlane = plane0.disToPlane(x)
            if (disToPlane > -1 and disToPlane < 1):   
                colPoints.append(x) 
        
        #if more than 3 points are touching the plane
        if(len(colPoints) > 3):
            #add impulse opposite of normal to cancel
            cube0.velocity -= cube0.velocity.project(plane0.normal)
            fg = cube0.gravity * cube0.mass
            # Fn = -cos(45)(m)(g)
            fn = -plane0.normal.normalized() * cube0.mass * cube0.gravity.length() * math.cos(plane0.angle)
            
            # -fg = cos()fn + sin()ff
            # ff = (-fg-cos()fn) / sin()
            ff = -fg - (fn * math.cos(plane0.angle)) / math.sin(plane0.angle)
            
            #finds max ff according to cof
            ffMax = fn * plane0.cof
            
            #if ff is larger than maxff then reduce ff to maxff
            if(ff.length() >= ffMax.length()):
                ff = ff.normalized() * ffMax.length() 
                
            cube0.gravity = LVector3(0, 0, 0)
        
        #v = v + at
        cube0.velocity = (cube0.gravity + (ff + fn + fg)/cube0.mass) * 1/fps + cube0.velocity
        print(ff)
        #print(cube0.velocity)
        cube0.gravity = gravityHold
